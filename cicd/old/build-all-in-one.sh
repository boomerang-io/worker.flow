...
BUILD_TOOL=${arr['build.tool']}
BUILD_TOOL_VERSION=${arr['build.tool.version']}
...
GIT_SSH_URL="${arr['component/repoSshUrl']}"
GIT_REPO_URL="${arr['component/repoUrl']}"
...
if  [ -d "$WORKSPACE_FOLDER" ]; then
    cd $WORKSPACE_FOLDER
    ls -ltr
    GIT_COMMIT_ID="${arr['git.commit.id']}"
...
IMAGE_NAME=${arr['docker.image.name']}
...
IMAGE_ORG=`echo ${arr['team.name']} | sed 's/[^a-zA-Z0-0]//g' | tr '[:upper:]' '[:lower:]'`
# Registry Host will potentially required a NO_PROXY entry in the controller service
REGISTRY_HOST=${arr['global/container.registry.host']}
REGISTRY_PORT=${arr['global/container.registry.port']}
REGISTRY_USER=${arr['global/container.registry.user']}
REGISTRY_PASSWORD=${arr['global/container.registry.password']}
