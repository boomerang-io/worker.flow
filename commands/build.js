...
module.exports = {
  async execute() {
    ...
    try {
      await exec(
        shellDir + "/common/initialize.sh " + taskProps["language.version"]
      );
      log.ci("Initializing Dependencies");
      if (
        taskProps["system.mode"] === "lib.jar" ||
        taskProps["system.mode"] === "java"
      ) {
        await exec(
          shellDir +
            "/common/initialize-dependencies-java.sh " +
            taskProps["language.version"]
        );
        await exec(
          shellDir +
            "/common/initialize-dependencies-java-tool.sh " +
        ...
        await exec(
          shellDir +
            "/common/initialize-dependencies-node.sh " +
            taskProps["build.tool"] +
            " " +
        ...
        taskProps["system.mode"] === "python" ||
        taskProps["system.mode"] === "lib.wheel"
      ) {
        await exec(
          shellDir +
            "/common/initialize-dependencies-python.sh " +
            taskProps["language.version"]
        );
      }
      ...
      await exec(
        shellDir +
          "/common/git-clone.sh " +
          taskProps["component/repoSshUrl"] +
          " " +
          taskProps["component/repoUrl"] +
          " " +
      ...
      if (taskProps["system.mode"] === "lib.jar") {
        ...
        await exec(
          shellDir +
            ...
            taskProps["build.tool.version"] +
            " " +
            taskProps["version.name"].substr(
              0,
              taskProps["version.name"].lastIndexOf("-")
            ) +
            " " +
        ...
        log.ci("Compile Artifact(s)");
        await exec(
          shellDir + "/build/compile-node.sh " + taskProps["build.tool"]
        );
  ...
  async lib_jar() {
    log.debug("Started CICD Lib Jar Build Activity");
    ...
    try {
      ...
      await exec(
        shellDir +
          ...
          taskProps["build.tool.version"] +
          " " +
          taskProps["version.name"].substring(
            0,
            taskProps["version.name"].lastIndexOf("-")
          ) +
          " " +
      ...
      log.debug("Finished CICD Lib Jar Build Activity");
    }
  }
};
