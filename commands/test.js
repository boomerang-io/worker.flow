  ...
  async execute() {
    ...
    try {
      if (!testTypes.length) {
        ...
        await exec(shellDir + "/common/git-clone.sh " + JSON.stringify(taskProps["component/git.private.key"]) + " " + taskProps["component/repoSshUrl"] + " " + taskProps["component/repoUrl"] + " " + taskProps["git.commit.id"]);
