...
module.exports = {
  async execute() {
    ...
    var testTypes =
      taskProps["test.type"] !== undefined && taskProps["test.type"] !== null ? taskProps["test.type"].split(",") : [];
      ...
      if (!testTypes.length) {
        ...
        if (taskProps["system.mode"] === "lib.jar" || taskProps["system.mode"] === "java") {
          if (!fileCommand.checkFileContainsStringWithProps("/data/workspace/pom.xml", "<plugins>", undefined, false)) {
            ...
            fileCommand.replaceStringInFileWithProps(
              "/data/workspace/pom.xml",
              "<plugins>",
              replacementString,
              undefined,
              false
          ...
          if (
            !fileCommand.checkFileContainsStringWithProps(
              "/data/workspace/pom.xml",
          ...
          await exec(
            shellDir +
              "/test/initialize-dependencies-node.sh " +
              taskProps["build.tool"] +
              " " +
              taskProps["node.package.script"] +
              " " +
              taskProps["node.cypress.install.binary"]
          );
          if (testTypes.includes("static")) {
            log.debug("Commencing static tests");
            await exec(
          ...
          if (testTypes.includes("static")) {
            ...
            await exec(
              shellDir +
                "/test/initialize-dependencies-static-python.sh " +
