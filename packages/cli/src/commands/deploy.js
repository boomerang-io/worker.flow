    ...
    try {
      ...
      if (taskProps["deploy.git.clone"]) {
        ...
        await exec(
          shellDir +
            '/common/git-clone.sh "' +
            taskProps["git.private.key"] +
            '" "' +
            JSON.stringify(taskProps["component/repoSshUrl"]) +
            '" "' +
            JSON.stringify(taskProps["component/repoUrl"]) +
            '" "' +
      ...
      if (taskProps["deploy.type"] === "kubernetes") {
        taskProps["process/container.port"] =
          taskProps["deploy.kubernetes.container.port"] !== undefined
            ? taskProps["deploy.kubernetes.container.port"]
            : "8080";
        taskProps["process/service.port"] =
          taskProps["deploy.kubernetes.service.port"] !== undefined
            ? taskProps["deploy.kubernetes.service.port"]
            : "80";
        ...
        var kubeFile =
          taskProps["deploy.kubernetes.ingress"] == undefined || taskProps["deploy.kubernetes.ingress"] == false
            ? "^kube.yaml$"
            : ".*.yaml$";
        if (taskProps["deploy.kubernetes.file"] !== undefined) {
          kubePath =
            taskProps["deploy.kubernetes.path"] !== undefined
              ? "/data/workspace" + taskProps["deploy.kubernetes.path"]
              : "/data/workspace";
          kubeFile = taskProps["deploy.kubernetes.file"];
        }
        var kubeFiles = await fileCommand.replaceTokensInFileWithProps(
          kubePath,
          kubeFile,
          "@",
          "@",
